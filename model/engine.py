...
class R2LEngine():
    ...
    def _save_instrinsics(self):
        near_value = self.dataset_info.near.item() if isinstance(self.dataset_info.near, torch.Tensor) else self.dataset_info.near
        far_value = self.dataset_info.far.item() if isinstance(self.dataset_info.far, torch.Tensor) else self.dataset_info.far
